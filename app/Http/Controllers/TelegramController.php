...
class TelegramController extends Controller
{
    ...
    public function handleWebhook(Request $request)
    {
        ...
        if (!$user) {
            $user = User::create([
                'telegram_id' => $from['id'],
                'name' => trim(($from['first_name'] ?? 'User') . ' ' . ($from['last_name'] ?? '')),
        ...
        if ($text === '/start' || $text === '/language') {
            ...
            // Updated to send the standard banner menu
    ...
    protected function handleCallback($callback)
    {
        ...
        if ($data === 'lang_select') {
            ...
            $this->sendTelegramRequest('answerCallbackQuery', ['callback_query_id' => $callback['id']]);
            return response()->json(['status' => 'success']);
        }
        ...
        if (strpos($data, 'setlang_') === 0) {
            ...
            $this->editPhotoToMainMenu($chatId, $messageId, $user, $newLang);
            
            $this->sendTelegramRequest('answerCallbackQuery', [
                'callback_query_id' => $callback['id'],
                'text' => ($newLang === 'am' ? "ቋንቋ ተቀይሯል" : "Language Updated")
            ]);
        }
    ...
    protected function editPhotoToMainMenu($chatId, $messageId, $user, $lang)
    {
        ...
        $this->sendTelegramRequest('editMessageCaption', [
            ...
            'reply_markup' => [
                'inline_keyboard' => [
                    [['text' => $texts['play'], 'web_app' => ['url' => "https://subconjunctive-tabetha-lotic.ngrok-free.dev?lang=$lang"]]],
                    [['text' => $texts['change_lang'], 'callback_data' => 'lang_select']]
                ]
            ]
        ]);
    }
    ...
    protected function sendUnknownCommandReply($chatId, $user)
    {
        ...
        $msg = ($lang === 'am') ? "❓ ይቅርታ፣ ትዕዛዙን አላወቅኩትም።" : "❓ Sorry, I didn't recognize that command.";
        $this->sendTelegramRequest('sendMessage', ['chat_id' => $chatId, 'text' => $msg]);

        // 2. Send the FULL photo menu (same as /start)
        $imageUrl = "https://api.lememar.com/storage/images/welcome-banner.jpg";
